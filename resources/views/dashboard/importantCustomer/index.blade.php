...
@section('css')
    ...
    @section('title')
        {{ trans('main.Important Customers') }}
    @stop
@endsection
...
@section('content')
    ...
    <div class="page-header">
        <div class="row align-items-center">
            <div class="col">
                <h3 class="page-title">{{ trans('main.Important Customers') }}</h3>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}">{{ trans('main.Dashboard') }}</a></li>
                    <li class="breadcrumb-item active">{{ trans('main.Important Customers') }}</li>
                </ul>
            </div>
            <div class="col-auto">
    ...
    <!-- Search Filter -->
    <div class="card filter-card" id="filter_inputs" @if($from_date || $to_date) style="display:block" @endif>
        <div class="card-body pb-0">
            <form action="{{ route('dashboard-importantCustomer.index') }}" method="get">
    ...
    <div class="row">
                            ...
                            <table id="example1" class="table table-stripped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center notPrint">
                                            @if($item->photo)
                                                <img loading="lazy" class="mb-1" src="{{ asset('public/attachments/importantCustomer/'.$item->photo) }}" alt="{{ $item->photo }}" height="50" width="70">
                                                <br>
                                                <a class="btn btn-outline-success btn-sm" href="{{ url('show_file') }}/importantCustomer/{{ $item->photo }}" role="button"><i class="fas fa-eye"></i></a>
                                                <a class="btn btn-outline-info btn-sm" href="{{ url('download_file') }}/importantCustomer/{{ $item->photo }}" role="button"><i class="fas fa-download"></i></a>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                            ...
                                            <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $item->id }}" title="{{ trans('main.Delete') }}" style="color:white !important"><i class="far fa-trash-alt"></i></button>
                                        </td>
                                    </tr>
                                    @include('dashboard.importantCustomer.editModal')
                                    @include('dashboard.importantCustomer.deleteModal')
                                    @endforeach
                                    @else
                                    <tr>
        ...
        @include('dashboard.importantCustomer.addModal')
        @include('dashboard.importantCustomer.deleteSelectedModal')		
    </div>
