    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h4 class="modal-title">{{ trans('main.Edit') }} {{ trans('main.Customers Review') }}</h4>
                <button type="button" class="close" data-bs-dismiss="modal"><span>&times;</span></button>
            </div>

            <div class="modal-body">
                <form action="{{ route('dashboard-customersReview.update', 'test') }}" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
                    {{ method_field('patch') }}
                    @csrf
                    <!--user_name_ar-->
                    <div class="form-group">
                        <label>{{ trans('main.User Name') }} {{ trans('main.In Arabic') }}</label>
                        <input class="form-control" placeholder="{{trans('main.User Name') }} {{ trans('main.In Arabic') }}" name="user_name_ar" value="{{ $item->user_name_ar }}" required>
                        <div class="valid-feedback">{{ trans('main.Looks Good') }}</div>
                        <div class="invalid-feedback">{{ trans('main.Error Here')}}</div>
                    </div>
                    <!--user_name_en-->
                    <div class="form-group">
                        <label>{{ trans('main.User Name') }} {{ trans('main.In English') }}</label>
                        <input class="form-control" placeholder="{{trans('main.User Name') }} {{ trans('main.In English') }}" name="user_name_en" value="{{ $item->user_name_en }}" required>
                        <div class="valid-feedback">{{ trans('main.Looks Good') }}</div>
                        <div class="invalid-feedback">{{ trans('main.Error Here')}}</div>
                    </div>
                    <!--content_ar-->
