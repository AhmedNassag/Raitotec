    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h4 class="modal-title">{{ trans('main.Add') }} {{ trans('main.Customers Review') }}</h4>
                <button type="button" class="close" data-bs-dismiss="modal"><span>&times;</span></button>
            </div>

            <div class="modal-body">
                <form action="{{ route('dashboard-customersReview.store') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                    @csrf
                    <!--user_name_ar-->
                    <div class="form-group">
                        <label>{{ trans('main.User Name') }} {{ trans('main.In Arabic') }}</label>
                        <input class="form-control" placeholder="{{trans('main.User Name') }} {{ trans('main.In Arabic') }}" name="user_name_ar" value="{{ old('user_name_ar') }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" placeholder="{{trans('main.User Name') }} {{ trans('main.In English') }}" name="user_name_en" value="{{ old('user_name_en') }}" required>
                        <div class="valid-feedback">{{ trans('main.Looks Good') }}</div>
                        <div class="invalid-feedback">{{ trans('main.Error Here')}}</div>
                    </div>
                    <!--content_ar-->
