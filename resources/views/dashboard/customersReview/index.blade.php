...
@section('content')
	...
	<div class="page-header">
		<div class="row align-items-center">
			<div class="col">
                <h3 class="page-title">{{ trans('main.Customers Reviews') }}</h3>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}">{{ trans('main.Dashboard') }}</a></li>
                    <li class="breadcrumb-item active">{{ trans('main.Customers Reviews') }}</li>
                </ul>
            </div>
			<div class="col-auto">
	...
	<!-- Search Filter -->
    <div class="card filter-card" id="filter_inputs" @if($user_name || $content || $from_date || $to_date) style="display:block" @endif>
        <div class="card-body pb-0">
            <form action="{{ route('dashboard-customersReview.index') }}" method="get">
                <div class="row filter-row">
                    <div class="col-sm-6 col-md-3">
                        <div class="form-group">
                            <label>{{ trans('main.User Name') }}</label>
	...
	<div class="row">
						...
						<table id="example1" class="table table-stripped">
							<thead>
								<tr>
									...
									<th class="text-center">{{ trans('main.User Name') }}</th>
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center notPrint">
										@if($item->photo)
											<img loading="lazy" class="mb-1" src="{{ asset('public/attachments/customersReview/'.$item->photo) }}" alt="{{ $item->photo }}" height="50" width="70">
											<br>
											<a class="btn btn-outline-success btn-sm" href="{{ url('show_file') }}/customersReview/{{ $item->photo }}" role="button"><i class="fas fa-eye"></i></a>
											<a class="btn btn-outline-info btn-sm" href="{{ url('download_file') }}/customersReview/{{ $item->photo }}" role="button"><i class="fas fa-download"></i></a>
										@endif
									</td>
								...
								@include('dashboard.customersReview.editModal')
								@include('dashboard.customersReview.deleteModal')
								@endforeach
								@else
								<tr>
        ...
        @include('dashboard.customersReview.addModal')
        @include('dashboard.customersReview.deleteSelectedModal')		
	</div>
@endsection