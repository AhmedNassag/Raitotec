...
@section('content')
	...
	<div class="page-header">
		<div class="row align-items-center">
			<div class="col">
                <h3 class="page-title">{{ trans('main.Service Reasons') }}</h3>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}">{{ trans('main.Dashboard') }}</a></li>
                    <li class="breadcrumb-item active">{{ trans('main.Service Reasons') }}</li>
                </ul>
            </div>
			<div class="col-auto">
    ...
    <div class="card filter-card" id="filter_inputs" @if($title || $content || $service_id || $from_date || $to_date) style="display:block" @endif>
        <div class="card-body pb-0">
            <form action="{{ route('dashboard-serviceReason.index') }}" method="get">
	...
	<div class="row">
						...
						<table id="example1" class="table table-stripped">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center notPrint">
										@if($item->photo)
											<img loading="lazy" class="mb-1" src="{{ asset('public/attachments/serviceReason/'.$item->photo) }}" alt="{{ $item->photo }}" height="50" width="70">
											<br>
											<a class="btn btn-outline-success btn-sm" href="{{ url('show_file') }}/serviceReason/{{ $item->photo }}" role="button"><i class="fas fa-eye"></i></a>
											<a class="btn btn-outline-info btn-sm" href="{{ url('download_file') }}/serviceReason/{{ $item->photo }}" role="button"><i class="fas fa-download"></i></a>
										@endif
									</td>
								...
								@include('dashboard.serviceReason.editModal')
								@include('dashboard.serviceReason.deleteModal')
								@endforeach
								@else
								<tr>
        ...
        @include('dashboard.serviceReason.addModal')
        @include('dashboard.serviceReason.deleteSelectedModal')		
	</div>
@endsection