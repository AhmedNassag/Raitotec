...
@section('content')
	...
	<div class="page-header">
		<div class="row align-items-center">
			<div class="col">
                <h3 class="page-title">{{ trans('main.Slider') }}</h3>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}">{{ trans('main.Dashboard') }}</a></li>
                    <li class="breadcrumb-item active">{{ trans('main.Slider') }}</li>
                </ul>
            </div>
			<div class="col-auto">
	...
	<div class="row">
						...
						<table id="example1" class="table table-stripped">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center notPrint">
										@if($item->photo)
											<img loading="lazy" class="mb-1" src="{{ asset('public/attachments/slider/'.$item->photo) }}" alt="{{ $item->photo }}" height="50" width="70">
											<br>
											<a class="btn btn-outline-success btn-sm" href="{{ url('show_file') }}/slider/{{ $item->photo }}" role="button"><i class="fas fa-eye"></i></a>
											<a class="btn btn-outline-info btn-sm" href="{{ url('download_file') }}/slider/{{ $item->photo }}" role="button"><i class="fas fa-download"></i></a>
										@endif
									</td>
								...
								@include('dashboard.slider.editModal')
								@include('dashboard.slider.deleteModal')
								@endforeach
								@else
								<tr>
        ...
        @include('dashboard.slider.addModal')
        @include('dashboard.slider.deleteSelectedModal')		
	</div>
@endsection